...
**Amendment 2026-07-31 — product framing consciously reopened.** Decision (b) in `cms-studio-outline.md` §9: OSS self-deploy plus operator-managed instances, via per-tenant provisioning (own worker + D1/R2/KV per customer — no multi-tenant D1; that threat stays parked). The differentiation answer the retirement demanded: agent-first markdown-native core, self-hostable on the customer's own Cloudflare, an owned derive pipeline enabling annotation-based visual editing *on markdown*, and the chat-as-editor loop — i.e. the markdown-vs-Portable-Text thesis moves from zero-stakes personal trial to the product's live wire. The kill criteria above are **not waived**: they remain the evaluation bar, and the EmDash-relative differentiation question (free, MIT, MCP-native, dashboard-distributed) still owes a maturing answer — currently: framework-agnostic derived-HTML contract (not Astro-tied), visual editing + studio + managed service on top of the same open core. Selectively un-parked onto the managed-instance track: pricing-when-users-are-agents, rate/anomaly breakers. Still parked: CRDT co-editing, enterprise vendor-lock objections, fleet-scale slice migration.
